  ...
  function initMilitaryCharCoder() {
    ...
    const ele = (t) => t ? document.createElement(t) : document.createElement('div');
    ...
    function topZIndexer() {
        return Math.ceil((new Date().getTime() - 2147483647)/1000) ;
    }
    ...
    function adjustElementSize() {
      ...
      function elementDrag(e) {
        ...
        let main_height =
          moved_height < min_height
            ? min_height
            : Math.floor(height - (pos4 - e.clientY) - foot_height);
        inlineStyler(
          cont,
          `{width: ${main_width}px;${moved_width < min_width ? "" : " height: " + main_height + "px; "}z-index: ${topZIndexer()};}`,
        );
        if (tbod) {
          inlineStyler(
            tbod,
            `{width: ${main_width + tbod_css.width}px;${moved_width < min_width ? "" : " height: " + (main_height + tbod_css.height) + "px;"}opacity: 0.5; }`,
          );
        }
      }
      function closeDragElement() {
    ...
    function createBasicContainer(params) {
      ...
      var page_70 = window.innerWidth * 0.7;
      var main_width = page_70 < 550 ? page_70 : 550;
      let cont = ele("div");
      a(
        cont,
        [
          ["class", `menu_container${params?.classname ? " " + params?.classname : ""}`],
          ["id", params?.id],
        ].filter((r) => r[1]),
      );
      inlineStyler(
        cont,
        `{position: fixed; top: ${params?.top ? params?.top + "px" : "0px"}; width: ${params?.width ? params?.width + "px" : main_width}px; left: ${params?.left ? params?.left + "px" : "0px"}; z-index: ${topZIndexer()}; background: ${params?.bg_color ? params?.bg_color : "transparent"}; border-radius: 0.2em; border: 2px solid ${params.border_color ? params.border_color : "transparent"}; box-shadow:0px 1px 1px 1px rgba(0,0,0,0.4);}`,
      );
      try {
      ...
      inlineStyler(
        header,
        `{height: 32px; display: grid; grid-template-columns: 32px 1fr; grid-gap:0px; user-select: none;}`,
      );
      ...
      inlineStyler(middle_cont, `{overflow: auto; display:grid; grid-template-columns:1fr 1fr; grid-gap:8px;}`);
      a(middle_cont, [
        ["class", "middle_cont"],
        ["data-css", btoaJSON({ height: -(26 + 32), width: -64 })],
      ]);
      ...
      inlineStyler(
        footer,
        `{height: 22px; display:grid; grid-template-columns: 1fr 22px; user-select: none;}`,
      );
      let foot_label = ele("div");
      footer.appendChild(foot_label);
      inlineStyler(foot_label, `{display: grid; grid-template-columns: 1fr 1fr;}`); 
      ...
      return {
        ...
        /* this is where you will attach all of the elements you wish to add to the container */ right_side:
          right_side,
        footer: footer,
    ...
    var left_mid = ele();
    cont_elms.middle_cont.appendChild(left_mid);
      
    var right_mid = ele();
    cont_elms.middle_cont.appendChild(right_mid);
    var outout_cont = ele();
    right_mid.appendChild(outout_cont);
    a(outout_cont,[['class','charlie_bravo_outout_table']]);
    inlineStyler(
      right_mid,
    ...
    left_mid.appendChild(textarea);
    inlineStyler(
      textarea,
      `{ background:#fffffe; color:#1c1c1c; padding:2px; width:100%; height:calc(100% - 14px);}`,
    );
    textarea.value = /(?<=\bstock=)\w{5,17}(?=&|$)|(?<=\bstock(num|number)=)\w{5,17}(?=&|$)|(?<=\bstock_(num|number)=)\w{5,17}(?=&|$)|(?<=\bstock[":\s]+")\w{5,17}(?=")|(?<=\bstock(num|number)[":\s]+")\w{5,17}(?=")|(?<=\bstock_(num|number)[":\s]+")\w{5,17}(?=")/i.exec(document.body.innerHTML)?.[0] || /(?<=\bvin[":\s]+")\w{11,17}(?=")/.exec(document.body.innerHTML)?.[0] || '';
    
      
    var go_btn = ele("div");
    go_btn.innerText = "Standard Conversion";
    inlineStyler(go_btn, `{margin: auto; text-align:center; cursor:pointer; color:#fffffe; border-radius: 0.2em; padding:2px; font-size:12px;}`);
    go_btn.onmouseenter = ()=> {inlineStyler(go_btn, `{background-color:#53fc18; color: ${container_params.bg_color};border: 0px solid #53fc18;}`);};
    go_btn.onmouseleave = ()=> {inlineStyler(go_btn, `{background-color:${container_params.bg_color}; color:#fffffe; border: 0px solid transparent;}`);};
    cont_elms.foot_label.appendChild(go_btn);

    var mfg_btn = ele("div");
    mfg_btn.innerText = "Car Name Conversion";
    inlineStyler(mfg_btn, `{margin: auto; text-align:center; cursor:pointer; color:#fffffe; border-radius: 0.2em; padding:2px; font-size:11px;}`);
    mfg_btn.onmouseenter = ()=> {inlineStyler(mfg_btn, `{background-color:#53fc18; color:${container_params.bg_color}; border: 0px solid #53fc18;}`);};
    mfg_btn.onmouseleave = ()=> {inlineStyler(mfg_btn, `{background-color:${container_params.bg_color}; color:#fffffe; border: 0px solid transparent;}`);};
    cont_elms.foot_label.appendChild(mfg_btn);
      
    var mfg_match = /ford|nissan|chevrolet|toyota|honda|mercedes/i.exec(window.location.href)?.[0]?.toLowerCase();

    go_btn.onclick = () => {
      if (textarea.value?.trim()?.length) {
        let val = stringToMilitaryLetterCallout({
            str:textarea.value?.trim(),
        });
        createOutputTable({table:val,parent:outout_cont})
      }
    };
    mfg_btn.onclick = () => {
      if (textarea.value?.trim()?.length) {
    ...
    function createOutputTable(params){
        var {table,parent}=params;
        if(document.getElementById('table_conversion')) {document.getElementById('table_conversion').remove()}
        var table_elm = ele('table');
        parent.appendChild(table_elm);
        a(table_elm,[['id','table_conversion']]);
        table.forEach((row,i)=> {
            let row_elm = ele('tr');
            if(i > table.length - 9){ a(row_elm,[['class','last_eight']]); }
            table_elm.appendChild(row_elm);
            row.forEach(cell=> {
                let cell_elm = ele('td');
                row_elm.appendChild(cell_elm);
                cell_elm.textContent = cell || '';
            })
        });
    }
    function setCSS(style_id,css_text){
    ...
    setCSS('charlie_bravo_css',`
    .charlie_bravo_outout_table:hover .last_eight {
      color: #53fc18;
    }
    .charlie_bravo_outout_table{
      color:#ffffff;
    }
  `);
    function stringToMilitaryLetterCallout(params) {
      var {str,mfg,btn_type} = params;
      var mfg_obj = { 
      'ford': {
        'A': "Aerostar",
        'B': "Bronco",
      ...
      return str.trim()
        .split('')
        .map((char) => {
          let word = mfg_obj[mfg] && mfg_obj[mfg][char.toUpperCase()] ? mfg_obj[mfg][char.toUpperCase()] 
          : btn_type == 'mfg_codes' && mfg_codes[char.toUpperCase()] ? mfg_codes[char.toUpperCase()]
          : codes[char.toUpperCase()] ? codes[char.toUpperCase()] : char;
          return [word];
        })
    }
  }
